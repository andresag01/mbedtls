...
int main( void )
{
     ...
     */
    retval = mbedtls_x509_crt_parse( &srvcert, (const unsigned char *) mbedtls_test_srv_crt,
    ...
    retval = mbedtls_x509_crt_parse( &srvcert, (const unsigned char *) mbedtls_test_cas_pem,
    ...
    retval =  mbedtls_pk_parse_key( &pkey, (const unsigned char *) mbedtls_test_srv_key,
    ...
    if( ( retval = mbedtls_ssl_conf_own_cert( &conf, &srvcert, &pkey ) ) != 0 )
    {
    ...
    if( ( retval = mbedtls_net_bind( &listen_fd, NULL, "4433", MBEDTLS_NET_PROTO_TCP ) ) != 0 )
    {
    ...
    while( 1 )
    {
        ...
        if( ( retval = mbedtls_net_accept( &listen_fd, &client_fd,
                                        NULL, 0, NULL ) ) != 0 )
        {
        ...
        if( pid != 0 )
        {
            ...
            if( ( retval = mbedtls_ctr_drbg_reseed( &ctr_drbg,
        ...
        while( ( retval = mbedtls_ssl_handshake( &ssl ) ) != 0 )
        {
        ...
        do
        {
            ...
            retval = mbedtls_ssl_read( &ssl, buf, len );
        ...
        while( cnt++ < 100 )
        {
            while( ( retval = mbedtls_ssl_write( &ssl, buf, len ) ) <= 0 )
            {
